...
import 'sphere_utils.dart';
import 'latlng.dart';

final mercator = MathUtil.mercator;
final wrap = MathUtil.wrap;
final hav = MathUtil.hav;
final clamp = MathUtil.clamp;
final arcHav = MathUtil.arcHav;
final havDistance = MathUtil.havDistance;
final inverseMercator = MathUtil.inverseMercator;
final havFromSin = MathUtil.havFromSin;
final sinSumFromHav = MathUtil.sinSumFromHav;
final sinFromHav = MathUtil.sinFromHav;
final computeDistanceBetween = SphericalUtil.computeDistanceBetween;
const double DEFAULT_TOLERANCE = 0.1; // meters.
...
class PolyUtil {
  ...
  static bool containsLocationFromLatLong(
  ...
  /*
     ...
     * With default tolerance to 0.1meter
     */
  ...
  /*
     ...
     * With default tolerance 0.1 meters
     */
  ...
  static List<LatLng> simplify(List<LatLng> poly, double tolerance) {
    ...
    if (n < 1) {
      throw ("Polyline must have at least 1 point");
    }
    if (tolerance <= 0) {
      throw ("Tolerance must be greater than zero");
    }
    ...
    int idx;
    int maxIdx = 0;
    Queue<List<int>> stack = Queue<List<int>>();
    List<double> dists = [];
    dists[0] = 1;
    dists[n - 1] = 1;
    double maxDist;
    ...
    if (n > 2) {
      ...
      while (stack.length > 0) {
        ...
        if (maxDist > tolerance) {
          ...
          stack.addLast(stackValCurMax);
          List<int> stackValMaxCur = [maxIdx, current[1]];
          stack.addLast(stackValMaxCur);
        }
      }
    }
  ...
  static double distanceToLine(
    ...
    LatLng sa =
        new LatLng(p.latitude - start.latitude, p.longitude - start.longitude);
  ...
  static String encode(final List<LatLng> path) {
    ...
    for (final LatLng point in path) {
      ...
      encodeWithBuffer(dLat, result);
      encodeWithBuffer(dLng, result);
  ...
  static void encodeWithBuffer(int v, StringBuffer result) {
    v = v < 0 ? ~(v << 1) : v << 1;
    while (v >= 0x20) {
